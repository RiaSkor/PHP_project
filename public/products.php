...
<div class="wrapper">
    ...
    <main>
        ...
        echo '<h2>Наша продукция</h2>';
        echo '<div class="containerP">';

        while ($product = $stmt->fetch()) {
            // Формируем описание типа как "surface, volume"
            $type = htmlspecialchars($product['surface'] . ', ' . $product['volume']);

            // Форматируем цену с пробелами (например, 1250 -> "1 250")
            $priceFormatted = number_format($product['price'], 0, ',', ' ');

            // Безопасный вывод данных
            $id = (int)$product['id_products'];
            $name = htmlspecialchars($product['name_products']);
            $priceData = (int)$product['price'];
            $imageFile = htmlspecialchars($product['image']) . '.png'; // добавляем расширение

            echo '<div class="product">';
            echo '<img src="static/products/' . $imageFile . '" alt="' . $name . '">';
            echo '<h2>' . $name . '</h2>';
            echo '<p class="type">' . $type . '</p>';
            echo '<p class="price">' . $priceFormatted . ' руб.</p>';
            echo '<button class="add-to-cart" data-id="' . $id . '" data-name="' . $name . '" data-price="' . $priceData . '">Добавить в корзину</button>';
            echo '</div>';
        }
